...
# kami akan mendeklarasikan definisi konstanta global

SPEED = 0.36
SNAKE_SIZE = 9
APPLE_SIZE = SNAKE_SIZE  # we will keep both food and size pf snake same
...
# kami akan menginisialisasi layar

screen = pygame.display.set_mode(
    (SCREEN_WIDTH, SCREEN_HEIGH), pygame.HWSURFACE)

# kami telah menggunakan 'hwsurface' yang merupakan singkatan dari permukaan perangkat keras yang mengacu pada penggunaan memori pada kartu video untuk menyimpan gambar sebagai lawan dari memori utama
...
def checkLimits(snake):
    if(snake.x > SCREEN_WIDTH):
        snake.x = SNAKE_SIZE
    if(snake.x < 0):  # this will be cheked  when some part of snake is on other side and some on opposite side
        snake.x = SCREEN_WIDTH - SNAKE_SIZE
    if(snake.y > SCREEN_HEIGH):
        snake.y = SNAKE_SIZE
    if(snake.y < 0):  # this also some half half
        snake.y = SCREEN_HEIGH - SNAKE_SIZE
...
# we will define moved of the snake
    ...
    # we will make function of crashing when snake eats itself
    ...
    def draw(self, screen):
        pygame.draw.rect(screen, pygame.color.Color(
            "green"), (self.stack[0].x, self.stack[0].y, SNAKE_SIZE, SNAKE_SIZE), 0)
        counter = 1
        while(counter < len(self.stack)):
            if(self.stack[counter].color == "NULL"):
                ...
                continue
            pygame.draw.rect(screen, pygame.color.Color(
                "yellow"), (self.stack[counter].x, self.stack[counter].y, SNAKE_SIZE, SNAKE_SIZE), 0)
            counter += 1
    ...
    for event in pygame.event.get():
        if event.type == pygame.KEYDOWN:
            ...
            # For exit
                ...
                return "exit"
            # jika kita ingin melanjutkan permainan kembali
            elif event.key == pygame.K_y:
                return "yes"
            # Jika kita ingin berhenti bermain
...
def respawnApple(apples, index, sx, sy):
    radius = math.sqrt((SCREEN_WIDTH/2*SCREEN_WIDTH/2 +
                       SCREEN_HEIGH/2*SCREEN_HEIGH/2))/2
    angle = 999
    while(angle > radius):
        angle = random.uniform(0, 800)*math.pi*2
        x = SCREEN_WIDTH/2 + radius * math.cos(angle)
        y = SCREEN_HEIGH/2 + radius * math.sin(angle)
        if(x == sx and y == sy):
            continue
...
# initialisation of snake
    ...
    mySnake = snake(SCREEN_WIDTH/2, SCREEN_HEIGH/2)
